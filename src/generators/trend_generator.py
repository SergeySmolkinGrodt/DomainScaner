...
from config import DEFAULT_TLDS, TREND_KEYWORDS_FILE
...
def generate_trend_domains():
    ...
    """
    try:
        with open(TREND_KEYWORDS_FILE, 'r') as f:
            trends = [line.strip() for line in f if line.strip() and not line.startswith('#')]
    except FileNotFoundError:
        print(f"Error: Trend keywords file not found at {TREND_KEYWORDS_FILE}")
        return []
